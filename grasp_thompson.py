    ...
    def build(self, alpha: float, rng: random.Random,
        ...
        for rank, e in enumerate(I.E):  # mais sênior -> menos sênior
            ...
            while sol.load[e] < target:
                ...
                # RCL de limiar (lista já ordenada → para cedo)
                RCL: List[Occ] = []
                for s in C:
                    v = I.v_es[(e, s)]
                    if v <= thr:
...
class ThompsonGRASP:
    ...
    def run(self) -> Tuple[ThompsonSolution, LexCost, List[float], int, int]:
        ...
        obj_history: List[float] = []   # evolução da FO
        last_improve_iter = 0
        total_iters = 0
        ...
        for it in range(1, self.iters + 1):
            ...
            total_iters = it
            obj_history.append(c.report_value)
            ...
            # EARLY STOP: todos os turnos foram alocados
            # if int(c.comp[0]) == 0:
            #    break

        assert best_sol is not None and best_cost is not None
        return best_sol, best_cost, obj_history, last_improve_iter, total_iters
...
def run_all_instances_and_save_csv(inst_dir: str,
    ...
    Campos principais no CSV:
      ...
      - obj_history: evolução da função objetivo ao longo das iterações
    """
    ...
    for fname in sorted(os.listdir(inst_dir)):
        ...
        for strategy in STRATEGIES:
            ...
            t0 = time.time()
            sol, cost, obj_history, conv_iter, total_iters = grasp.run()
            t1 = time.time()
            elapsed = t1 - t0
            ...
            # serializar histórico da FO (pode ficar longo, mas é fiel)
            obj_history_str = ";".join(f"{v:.4f}" for v in obj_history)

            row: Dict[str, object] = {
                "instance": instance_name,
                "strategy": strategy,
                "iterations_param": grasp.iters,
                ...
                "conv_iter": conv_iter,        # "número médio de iterações até convergência"
                "obj_history": obj_history_str # evolução da FO
            }
