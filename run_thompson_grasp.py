# run_thompson_grasp.py
from thompson_instance import ThompsonInstance
from thompson_grasp import ThompsonGrasp

# ==== cole os dicionários exatamente como você enviou ====
day_map = {1:'Sun',2:'Mon',3:'Tue',4:'Wed',5:'Thu',6:'Fri',7:'Sat'}

# (cole aqui) employee_data, shift_data, shift_requirements  ⬇⬇⬇python run_thompson_grasp.py
...
if __name__ == "__main__":
    ...
    grasp = ThompsonGrasp(instance=I, alpha=0.3, iterations=100, construction='traditional')
    best = grasp.solve()

    # Reconstrói a alocação final a partir de best
    ...
    # assign: s -> e
    assign = ev.assign
    unalloc = [s for s in I.S if assign[s] is None]

    # imprime por empregado
    alloc_by_emp = {e: [] for e in I.E}
    for s, e in assign.items():
        if e is not None:
            alloc_by_emp[e].append(s)

    print("\n--- Alocações por Empregado ---")
    for e in I.E:
        lst = alloc_by_emp[e]
        print(f"  {e} ({len(lst)} / {I.m_e[e]}): {lst}")

    print("\n--- Turnos NÃO alocados ---")
    if unalloc:
        for s in unalloc:
            print(f"  (Dia: {s[0]}, ID: {s[1]})")
    else:
        print("  Todos os turnos foram alocados!")

    print(f"\nCusto final (escalarização lexicográfica): {best.cost}")
